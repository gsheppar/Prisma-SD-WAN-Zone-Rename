...
def update_zones(cgx, list_of_csv):
    
    zone_list = []
    for item in cgx.get.securityzones().cgx_content['items']:
        for zone in list_of_csv:
            if zone["New_Name"] and zone["Current_Name"] == item["name"]:
                item["name"] = zone["New_Name"]
                
                resp = cgx.put.securityzones(securityzone_id=item["id"],data=item)
                if not resp:
                    print("ERROR updating zone " + item["name"])
                    print(str(jdout(resp)))
                else:
                    print("Updating zone name " + zone["Current_Name"] + " to " +  zone["New_Name"])
...
def go():
    ...
    list_of_csv = []
    with open('Zones.csv', 'r') as data:
      for line in csv.DictReader(data):
          list_of_csv.append(line)
    
    cgx = cgx_session
    update_zones(cgx, list_of_csv)
    return
